...
| attributes.**status** | `string` | The import job status. One of 'pending' \(default\), 'in\_progress', 'interrputed', or 'completed' |
...
| attributes.**errors\_log** | `array` | Contains the import errors, if any. |
| attributes.**warnings\_log** | `array` | Contains the import warnings, if any. |
